            ...
            <div className="tab-content-body">
              ...
              {activeTab === 'general' && (
                <>
                  ...
                  <div className="subsection-title">데이터 불러오기 방식</div>

                  <div className="info-notice" style={{ margin: '0 24px 16px' }}>
                    <div className="info-icon">💡</div>
                    <div className="info-text">
                      <strong>다중 선택 규칙</strong>
                      <p style={{ margin: '4px 0 0' }}>
                        • 여러 방식을 동시에 선택할 수 있습니다<br/>
                        • <strong>LIMS(.plrn)과 CSV는 동시에 선택할 수 없습니다</strong><br/>
                        • 바코드는 단독 사용이 불가하며, 다른 방식과 함께 선택해야 합니다
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="setting-control">
                      <div className="checkbox-group">
                        {[
                          ...
                          <div 
                            key={opt.key} 
                            className={`checkbox-item ${settings.loadingMethods[opt.key] ? 'checked' : ''}`}
                            onClick={() => {
                              const newMethods = { ...settings.loadingMethods, [opt.key]: !settings.loadingMethods[opt.key] };
                              // 바코드만 선택된 상태 방지
                              if (!newMethods.manual && !newMethods.preset && !newMethods.plrn && !newMethods.csv && newMethods.barcode) {
                                newMethods.barcode = false;
                              }
                          ...
                          >
                            ...
                            <div>
                              <div>{opt.label}</div>
                              <small style={{ color: '#64748b', fontSize: '11px' }}>{opt.desc}</small>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* 파일 기반 방식 - LIMS, CSV (택1) */}
                  <div className="setting-row">
                    <div className="setting-label">
                      파일 기반 방식
                      <small>LIMS와 CSV 중 택 1</small>
                    </div>
                    <div className="setting-control">
                      <div className="radio-group">
                        {[
                          { value: 'none', label: '사용 안 함' },
                          { value: 'plrn', label: 'LIMS(.plrn) 파일' },
                          { value: 'csv', label: 'CSV' }
                        ].map(opt => (
                          <div 
                            key={opt.value} 
                            className={`radio-item ${
                              opt.value === 'none' 
                                ? (!settings.loadingMethods.plrn && !settings.loadingMethods.csv ? 'selected' : '')
                                : (settings.loadingMethods[opt.value] ? 'selected' : '')
                            }`}
                            onClick={() => {
                              ...
                              if (opt.value === 'none') {
                                ...
                                // LIMS와 CSV 상호 배타적
                                newMethods.plrn = opt.value === 'plrn';
                                newMethods.csv = opt.value === 'csv';
                              }
                              // 바코드만 선택된 상태 방지
                              if (!newMethods.manual && !newMethods.preset && !newMethods.plrn && !newMethods.csv && newMethods.barcode) {
                                newMethods.barcode = false;
                  ...
                  <div className="setting-row">
                    <div className="setting-label">
                      추가 옵션
                      <small>바코드 스캔 기능</small>
                    </div>
                    <div className="setting-control">
                      {(() => {
                        const hasOtherMethod = settings.loadingMethods.manual || settings.loadingMethods.preset || 
                                               settings.loadingMethods.plrn || settings.loadingMethods.csv;
                        const isDisabled = !hasOtherMethod;
                        return (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            <div 
                              className={`checkbox-item ${settings.loadingMethods.barcode ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}`}
                              onClick={() => {
                                if (!isDisabled) {
                                  updateSetting('loadingMethods', { 
                                    ...settings.loadingMethods, 
                                    barcode: !settings.loadingMethods.barcode 
                                  });
                                }
                              }}
                            ...
                            >
                              ...
                              <div>
                                <div>바코드로 불러오기</div>
                                <small style={{ color: '#64748b', fontSize: '11px' }}>Plate Barcode 스캔으로 불러오기</small>
                              </div>
                            </div>
                            {isDisabled && (
                              <div style={{ 
                                ...
                                <span>⚠️</span>
                                <span>바코드 사용 시 기본 방식 또는 파일 기반 방식 중 하나 이상을 먼저 선택하세요</span>
                              </div>
                            )}
                          </div>
                        );
                    ...
                    <div className="setting-control">
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                        ...
                        {!Object.values(settings.loadingMethods).some(v => v) && (
                          <span style={{ color: '#ef4444', fontSize: '13px' }}>⚠️ 최소 하나의 방식을 선택하세요</span>
                        )}
                      </div>
                    </div>
                  </div>
