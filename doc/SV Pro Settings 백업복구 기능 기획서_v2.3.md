...
- **파일명**: `YYYY-MM-DDTHH-mm_settings_auto.json`
...
- **파일명**: `YYYY-MM-DDTHH-mm_settings_scheduled.json`
...
- **흐름**: 파일 선택 대화상자 → 파일 선택 → Diff 모달 → 적용
- **특이사항**:
  - Diff 모달에서 "동일" 상태 항목은 표시하지 않음 ("변경"과 "스킵"만 표시)
  - 변경할 항목이 없는 경우 (모두 동일): Diff 모달 표시 안 함, "동일한 설정 내용입니다" alert 팝업 표시
...
- **필터링**: "동일" 상태 항목은 표시하지 않음 ("변경"과 "스킵"만 표시)
- **크기**: 1200px 너비
...
3. **Diff 생성**: 현재(settings state) vs 파일 비교 ("동일" 상태 필터링)
4. **변경사항 확인**: 
   - 변경 또는 스킵 항목 있음: Confirm 팝업 표시
   - 변경할 항목 없음 (모두 동일): "동일한 설정 내용입니다" alert 팝업 표시 후 종료
...
│  ├─────────────────────────────────────────────────────┤  │
  │ 📄 2026-01-13T15-30_settings_auto.json          │  │
  │    2026-01-13 15:30:00 · 24KB          [복원]      │  │
  │ 📄 2026-01-13T14-45_settings_auto.json          │  │
...
│  ├─────────────────────────────────────────────────────┤  │
  │ 📄 2026-01-13T09-00_settings_scheduled.json     │  │
...
│  File: 2026-01-13T14-45_settings_auto.json          │
├──────────────────────────────────────────────────────────┤
...
### 6.2 Import
- **동일한 설정**: "동일한 설정 내용입니다." (alert 팝업)
- **파일 선택**: Diff/Confirm 팝업 자동 표시 ("변경" 또는 "스킵" 항목만 표시)
...
```json
{
  ...
  "fileName": "2026-01-13T15-30_settings_auto.json",
...
{
  "type": "import",
  "fileName": "2026-01-13T14-45_settings_auto.json",
...
- ✅ Import 중복 파일 감지 제거 (같은 파일 재선택 허용)
- ✅ Diff/Confirm 팝업에서 "동일" 상태 필터링 ("변경"과 "스킵"만 표시)
- ✅ 변경사항 없을 때 "동일한 설정 내용입니다" alert 팝업
- ✅ 보수적 호환 정책 (스킵 사유 표기)
...
- ✅ 중복 파일 감지 기능 제거 (v2.4부터)
- ✅ 동일한 설정 시 "동일한 설정 내용입니다" alert 팝업
...
| 2.4 | 2026-01-15 | Import 중복 파일 감지 제거, Diff 모달에서 "동일" 상태 필터링, 변경사항 없을 때 안내 팝업 추가 | - |
